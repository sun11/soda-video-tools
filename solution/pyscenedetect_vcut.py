...
tmp_file_dir = "./tmp"
DEFAULT_FFMPEG_ARGS = '-c:v libx264 -preset veryslow -crf 18 -c:a aac'
...
def get_video_list_from_jsonl(filepath):
    with open(filepath, 'r') as f:
        data = f.readlines()
    video_list = [json.loads(json_obj)['videos'][0] for json_obj in data]
    return video_list
...
def split_video_into_scenes(
    ...
    for idx in range(len(frame_points) - 1):
        ...
        output_scene_list.append((
            ...
            frame_timecode[frame_points[idx+1]]))
    if output_scene_list:
        random.seed(42)
        random.shuffle(output_scene_list)
        output_scene_list = output_scene_list[:10]
        # output_scene_list = sorted(output_scene_list, key=lambda x: x[1].get_frames() - x[0].get_frames(), reverse=True)[:4]
...
def main():
    ...
    video_list = get_video_list_from_jsonl(video_input)
    args_list = [
        (video, threshold, frame_skip, min_seconds, max_seconds, save_dir, name_template, save_json)
        for video in video_list
    ]

    with Pool(processes=num_processes, maxtasksperchild=8) as pool:
